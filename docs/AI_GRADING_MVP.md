...
产品核心：**判对错 + 输出解题过程 + 建知识点记忆库**，对应三个门控。

| 指标 | 最低要求 | 理想目标 | 说明 |
|------|---------|---------|------|
| `schema_parse_success` | ≥ 95% | 100% | 结构化输出稳定性 |
| `grading_accuracy` | ≥ 80% | ≥ 90% | AI 判断对错是否准确（核心） |
| `taxonomy_top1_hit` | ≥ 75% | ≥ 85% | 知识点映射命中率（记忆库质量） |

### 观测指标（不作为上线门槛）

| 指标 | 观测目标 | 说明 |
|------|---------|------|
| `answer_accuracy` | 记录 | AI 解题能力参考；批改本身是语义判断，不依赖字符串匹配 |

...
class StudentProfileService:
    ...
    async def get_profile(self, student_id: str) -> dict:
        ...
        result = await self.db.execute(
            ...
            .order_by(StudentKnowledgeProfile.mastery_level.asc())
        )
        ...
        total_correct = sum(p.correct_questions for p in profiles)
        correct_rate = total_correct / total_questions if total_questions > 0 else 0.0
        
        # 构建知识点列表
        knowledge_points = [
            {
                "name": p.knowledge_point,
                "mastery_level": p.mastery_level,
                "question_count": p.total_questions,
                "correct_count": p.correct_questions,
                "last_practice": p.last_practice_at.isoformat() if p.last_practice_at else None,
            }
            for p in profiles
        ]
    ...
    async def get_weak_points(
        ...
        result = await self.db.execute(
            ...
            .where(
                ...
                StudentKnowledgeProfile.mastery_level < threshold,
            )
            .order_by(StudentKnowledgeProfile.mastery_level.asc())
            .limit(limit)
        )
        ...
        for kp_name in question.knowledge_points:
            ...
            if not profile:
                ...
                profile.mastery_level = (
                    alpha * new_score + (1 - alpha) * profile.mastery_level
                )
                profile.last_practice_at = datetime.now()
    ...
    async def get_review_suggestions(
        ...
        for idx, wp in enumerate(weak_points):
            # 计算优先级
            priority = "high" if wp["mastery_level"] < 0.4 else "medium"
            
            # 计算推荐题目数（掌握度越低，推荐越多）
            recommended_questions = max(3, int((1 - wp["mastery_level"]) * 10))
            ...
            tasks.append({
                ...
                "priority": priority,
                "reason": f"掌握度低（{wp['mastery_level']:.0%}），需要重点复习",
